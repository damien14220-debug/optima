...
export default function CompteJoint({ user }) {
  ...
  useEffect(() => {
    if (TOUS_MOYENS.length > 0 && !form.moyen_paiement) {
      setForm(f => ({ ...f, moyen_paiement: TOUS_MOYENS[0] }))
    }
  }, [moyensDB])
  ...
  const s = { input: { width: '100%', padding: '8px 12px', borderRadius: 8, fontSize: 14, border: '1px solid var(--color-border)', background: 'var(--color-bg)', color: 'var(--color-text)' }, card: { background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 16 } }
  ...
  return (
    <div style={{ padding: 16, maxWidth: 800, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 16 }}>
        {[
          { label: 'Total', value: totalJoint, color: '#ec4899' },
